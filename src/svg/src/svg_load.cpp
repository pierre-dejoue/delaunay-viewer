...
void parse_ssvg_image_shape_list(const ssvg::ShapeList* shape_list_ptr, const SVGImageGeometry& image_geometry, Paths<F>& out_paths, const stdutils::io::ErrorHandler& err_handler)
{
    for (unsigned int shape_idx = 0; shape_idx < shape_list_ptr->m_NumShapes; shape_idx++)
    {
        ...
        switch (shape_type)
        {
            ...
            {
                const ssvg::ShapeList* group_shape_list = ssvg::shapeListGetGroupShapeList(shape_list_ptr, shape_idx);
                parse_ssvg_image_shape_list(group_shape_list, sub_image_geometry, out_paths, err_handler);
                break;
            }
...
Paths<F> parse_svg_paths_gen(std::filesystem::path filepath, const stdutils::io::ErrorHandler& err_handler) noexcept
{
    ...
    try
    {
        ...
        SVGImageGeometry src_image_geometry(
            &imageAttrs->m_Transform[0],
            ssvg::imageGetWidth(ssvg_img.ptr),
            ssvg::imageGetHeight(ssvg_img.ptr));
        Paths<F> result;
        const ssvg::ShapeList* root_shape_list = ssvg::imageGetRootShapeList(ssvg_img.ptr);
        parse_ssvg_image_shape_list(root_shape_list, src_image_geometry, result, err_handler);
        return result;
    }
